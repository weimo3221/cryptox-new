...
def get_device_with_most_free_memory():
    """
    Get the CUDA device with the most free video memory, if there is no available GPU, return CPU
    """
    ...
    for device in devices:
        ...
        # Get the total video memory of the current device
        total_mem = torch.cuda.get_device_properties(device).total_memory
        # Get the allocated video memory of the current device
        allocated_mem = torch.cuda.memory_allocated(device)
        # Calculate free video memory
        free_mem = total_mem - allocated_mem
        device_memory.append((device, free_mem))
    
    # Sort by free video memory in descending order
    device_memory.sort(key=lambda x: x[1], reverse=True)
    # Returns the device with the most free video memory
...
class AttnWrapper(torch.nn.Module):
    """
    Packaging the Attention Layer (Attn) for intervention and activation management
    """
    ...
    def forward(self, *args, **kwargs):
        """
        Forward propagation, modifying attentional behavior to achieve intervention
        """
...
class MLPWrapper(torch.nn.Module):
    """
    Packaging MLP layers for neuronal intervention and activation management
    """
    ...
    def forward(self, x):
        """
        Forward propagation, calculating the output via MLP and applying the intervention
        """
    ...
    def reset(self):
        """
        Reset Neuron Intervention Setting
        """
        self.neuron_interventions = {}
    
    def freeze_neuron(self, neuron_idx, mean):
        """
        Freeze the specified neuron and set its output to the given mean value
        """
        self.neuron_interventions[neuron_idx] = mean

class BlockOutputWrapper(torch.nn.Module):
    """
    Packaging the output of the Transformer layer to intervene and manage the activation and output of the layers
    """
    ...
    def block_add_to_last_tensor(self, tensor):
        """
        Add the intervention tensor to the output of the last token
        """
        self.add_to_last_tensor = tensor

    def attn_add_tensor(self, tensor):
        """
        Adding the intervention tensor to the attention output
        """
    ...
    def get_attn_activations(self):
        """
        Get the current layer's attentional activation value
        """
        return self.block.self_attn.activations

class LlamaHelper:
    """
    Provides a variety of functions to manipulate the model, such as generating text, intervening neurons, etc.
    """
    ...
    def generate_text(self, prompt, max_length=100):
        """
        Generates text based on a given prompt, up to a maximum length of max_length.
        """
    ...
    def sample_next_token(self, logits, temperature=1.0):
        """
        Sampling the next token, using a temperature-adjusted sampling strategy
        """
    ...
    def _sample_greedy(self, logits):
        """
        Greedy sampling strategy, choosing the maximum value as the next token
        """
        return logits.argmax().item()

    def _sample_basic(self, logits):
        """
        Base sampling strategy, sampling based on probability distribution
        """
        return torch.distributions.categorical.Categorical(logits=logits).sample().item()
    
    def get_logits(self, prompt):
        """
        Get logits (probability distribution of output) for a given prompt
        """
        inputs = self.tokenizer(prompt, return_tensors="pt")
    ...
    def set_neuron_intervention(self, layer_idx, neuron_idx, mean):
        """
        Intervene on a neuron in a specific layer by freezing the neuron and setting the mean value
        """
        self.model.model.layers[layer_idx].mlp_freeze_neuron(neuron_idx, mean)

    def set_add_attn_output(self, layer, add_output):
        """
        Adding additional output to layer-specific attention mechanisms
        """
        self.model.model.layers[layer].attn_add_tensor(add_output)

    def get_attn_activations(self, layer):
        """
        Get layer-specific attentional activations
        """
        return self.model.model.layers[layer].get_attn_activations()

    def set_add_to_last_tensor(self, layer, tensor):
        """
        Adds an additional tensor to the last token of a specific layer
        """
    ...
    def print_decoded_activations(self, decoded_activations, label):
        """
        Print the decoded activation value
        """
    ...
    def logits_all_layers(self, text, return_attn_mech=False, return_intermediate_res=False, return_mlp=False, return_block=True):
        """
        Get logits for all layers and return information about them.
        """
